...
const stages: StoryStage[] = ['barrier', 'reflection', 'action', 'transformation'];
...
export function MirrorSelector({ selected, onSelect }: MirrorSelectorProps) {
  return (
    <div role="group" aria-label="Where are you in your career story?">
      <p className="text-muted-foreground mb-4 text-lg">Where are you right now?</p>
          ...
          return (
            <Button
              ...
              className={cn(
                ...
                isActive && 'bg-golden text-golden-foreground hover:bg-golden/90 border-golden',
                !isActive && 'hover:border-golden/50 hover:scale-[1.02]'
              )}
            >
              {STAGE_LABELS[stage]}
            </Button>
